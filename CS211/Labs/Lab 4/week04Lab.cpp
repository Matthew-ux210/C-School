...
double geometricSumHelper(unsigned int n, double acc) {
    if (n == 0)
        return acc + 1;
    return geometricSumHelper(n - 1, (acc + 1) / 2);
}

double geometricSum(unsigned int n) {
    return geometricSumHelper(n, 0.0);
}
...
double smallestElementHelper(double arr[], int size, int i, double min) {
    if (i == size)
        return min;
    if (arr[i] < min)
        min = arr[i];
    return smallestElementHelper(arr, size, (i + 1), min);
}

double smallestElement(double arr[], int size) {
    if (size <= 0)
        return 0;
    return smallestElementHelper(arr, size, 1, arr[0]);
}
